...
const hasEveryInsertUserFields = (input: object) => {
	return insertUserAllowedFields.every(
...
const hasOnlyInsertUserFields = (input: object): input is InsertUser => {
	return Object.keys(input).every((key) =>
		insertUserAllowedFields.includes(key),
	);
};
...
const isUserInsertValid = (input: unknown): input is InsertUser => {
	return (
		isObject(input) &&
		hasEveryInsertUserFields(input) &&
		hasOnlyInsertUserFields(input)
	);
};
